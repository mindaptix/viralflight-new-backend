...
`GROQ_MODEL` is optional; the value above is the default. Restart the backend after setting the key and deploying the updated code. A key's display name in the Groq console is not its secret value.
